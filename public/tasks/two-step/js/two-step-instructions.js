...
const quiz_1 = {
  ...
  prompts: [
    ...
    "<i>True</i> or <i>False</i>:&nbsp;ある宇宙人は、ほかの宇宙人よりも「あたり」を頻繁にわたします。",
  ],
  options: [
...
const instructions_1b_node = {
  timeline: [{
    type: jsPsychTwoStepInstructions,
    pages: [
      "<p>お見事です。次は実際に宇宙人と体験しましょう。</p><p>次の画面で<b>左/右の矢印キーを使って</b>取引する宇宙人を選択します。どの宇宙人が「あたり」の「石」を渡す可能性が高いか判断するチャンスが 10 回あります。</p>",
...
const instructions_2a = {
  ...
  pages: [
    ...
    `<p>惑星を訪問するには、旅するロケットを選びます。</p><p>以下に、選択できるロケットの例を 2 つ示します:<br>the <b><font color='${practice_info.font_colors[0]}'>${practice_info.rocket_names[0]}</font></b> and <b><font color='${practice_info.font_colors[1]}'>${practice_info.rocket_names[1]}</font></b> rocket ships.</p>`,
    `<p>各ロケットには、「たいてい」は、到着する惑星がきまっています。</p><p><b><font color='${practice_info.font_colors[0]}'>${practice_info.rocket_names[0]}</font></b>このロケットならば、あの宇宙人と出会える、と「たいていは」わかっているのです。</p><p>この <b><font color='${practice_info.font_colors[1]}'>${practice_info.rocket_names[1]}</font></b>もしくは、あるロケットは「たいてい」は、別の惑星の宇宙人のところへ行くのです。</p>`,
...
const instructions_2b = {
  ...
  pages: [
    ...
    "<p><b>覚えておいてください:</b> ロケットは「たいていは」きまった星に到着しますが、</p><p>ときどき他の惑星に連れていきます。</p>",
...
const instructions_3a = {
  ...
  pages: [
    ...
    "「報酬の宇宙群」ではできる限り多くの「あたり」<b>（プラス１０円）</b>をあつめ、「罰則の宇宙群」では「あたり」<b>（マイナス１０円）</b>を回避するために、<b>以下に３つの手がかりがあります。</b></p><p>最後なので、注意してお読みください。</p>",
  ...
  on_start: function(trial) {
    ...
    if (jsPsych.data.get().filter({quiz: 3}).count() == 0) {
      trial.pages.unshift(
        "<p>以上でインストラクション終了です。</p><p>では、最後の確認テストです。</p>",
      )
    }
