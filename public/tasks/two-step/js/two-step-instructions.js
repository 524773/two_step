...
const instructions_1a = {
  ...
  pages: [
    ...
    "<p>「泥だんご」は報酬の宇宙群でも罰則の宇宙群でも、</p><p>「プラス」にも「マイナス」にもなりません。</p><p>「泥だんご」はこちらです。</p>",
    ...
    "<p>あなたの目標は、ある時期に最も「あたり」をわたす可能性のある宇宙人を見つけること。<b>つまり、「あたり」がプラスの「報酬の宇宙群」では、彼らとの取引をふやす選択をし、反対に、「あたり」がマイナスの「罰則の宇宙群」では、彼らとの取引を避けることです。</b></p>",
  ],
...
const instructions_1b_node = {
  timeline: [{
    type: jsPsychTwoStepInstructions,
    pages: [
      "<p>お見事です。次は実際に宇宙人と体験しましょう。</p><p>次の画面で<b>左/右の矢印キーを使って</b>取引する宇宙人を選択します。</p><p>どの宇宙人が「あたり」の「石」を渡す可能性が高いか試行してください。</p>",
    　 "<p><b>ヒント:</b>宇宙人はときどき画面にあらわれる<br>位置をかえてきます。位置がかわっても<br>「石」を渡す確率は変わりません。</p>",
      "<p>各選択の場面において、その都度１０秒以内に矢印キーで決定してください。５回以上時間切れがありますと打ち切られ、最初からになります。</p>",
    ],
...
const instructions_3a = {
  ...
  pages: [
    ...
    "<p>あなたの目的はいつ（時間の経過とともに）、どの宇宙人が「あたり」を渡してくるか注意してみつけることです。</p>",
  ...
  on_start: function(trial) {
    ...
    if (jsPsych.data.get().filter({quiz: 3}).count() == 0) {
      trial.pages.unshift(
        ...
        "<p>報酬の宇宙群に移動した方は謝礼金の基本は１，２００円です。（増やせるからです）</p>",
        "<p>罰則の宇宙群に移動した方は謝礼金の基本は１，５００円です。（減ってしまうからです）</p>",
        "<p>どちらになるかはランダムですが、同じ程度の金額が期待されますので、がんばって下さい。</p>",
      )
    }
