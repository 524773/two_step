...
const instructions_2a = {
  ...
  on_start: function(trial) {
    ...
    if (jsPsych.data.get().filter({quiz: 2}).count() == 0) {
      trial.pages.unshift(
        "<p>いかがですか。どの宇宙人が好条件の取引をする可能性が高いか、わかったのではないでしょうか</p>"
      )
    }
...
const instructions_3a = {
  ...
  pages: [
    ...
    "<p><b>Hint #1:</b>宇宙人が「好条件の取引」をする確率は時間の経過とともに変化しますが、この変化はゆっくりです。そのため、どの宇宙人が、好条件の取引をしているかを覚えておく必要があります。今あなたに好条件の取引をする宇宙人は、しばらく続ける可能性が高いからです。</p>",
...
const quiz_3 = {
  type: jsPsychTwoStepComprehension,
  prompts: [
    "<i>True</i> or <i>False</i>:&nbsp;&nbsp;宇宙人が「好条件の取引」をする確率は、時間の経過とともに変化する。",
    "<i>True</i> or <i>False</i>:&nbsp;&nbsp;宇宙人が「好条件の取引」をする確率は、選択したロケットによって異なる。",
