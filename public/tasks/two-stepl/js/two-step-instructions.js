...
const instructions_1a = {
  ...
  pages: [
    ...
    "<p>通訳の「ワープさん」は、</p><p>あなたが取引で損をすると<b>あなたに向けて「ぷんぷん顔」をみせます。</b></p><p>表情はこちらです。（画像は練習用です）</p>",
    "<p>プラスマイナス０の取引結果になれば</p><p>現状維持なので、「真顔」になります。</p><p>真顔はこちらです。（画像は練習用です）</p>",
    "<p>取引する宇宙人を選ぶには、キーボードの</p><p><b>左←か右→の矢印</b>キーを使います。</p>",
    "<p>宇宙人たちは<b>「毎回かならず」、</b></p><p>あなたにとって「悪条件の取引（マイナス１０円）」をする<b>わけではありません。</b></p><p>ときおりプラスマイナス０の取引をします。</p>",
    "<p>宇宙人たちは<b>ある時期、ほかの宇宙人よりも</b></p><p><b>頻繁にあなたに悪条件の取引をする期間があります。</b></p><p>つまり、その時期を把握すれば、回避することで<b>多くの損失を</b>、</p><p><b>避けられます。</b></p>",
...
const instructions_1b_node = {
  timeline: [{
    type: jsPsychTwoStepInstructions,
    pages: [
      "<p>お見事です。実際に宇宙人と取引するには、最初に搭乗ロケットを選択し、到着さきの宇宙人を選択する２段階の選択が必要になりますが、ここでは先に後半部分の宇宙人との取引を体験しましょう。</p><p>次の画面で<b>左/右の矢印キーを使って</b>取引する宇宙人を選択します。</p><p>どの宇宙人が「悪条件の取引」をするか試行してください。</p>",
    　 "<p><b>ヒント:</b>宇宙人はときどき画面にあらわれる<br>左右の位置をかえてきます。左右がかわっても<br>あなたにとって「悪条件の取引（－１０円）」をする確率は変わりません。</p>",
...
const practice_1_help_node = {
  timeline: [{
    type: jsPsychTwoStepInstructions,
    pages: [
      "<p>お困りですか。</p><p>覚えておいてください。<b>たいていの場合に</b>あなたにとって悪条件の取引をする宇宙人を<br><b>避けるのです。</b></p>",
...
const instructions_2a = {
  type: jsPsychTwoStepInstructions,
  pages: [
    "<p>次に、搭乗するロケット選びです。宇宙人と取引するためには２段階の選択 :（１）訪問するロケットを選ぶ。（２）取引する宇宙人を選ぶ。が必要です。★　さきほどの宇宙人との取引は、（２）の部分でした。</p>",
    `<p>今度は、（１）の、訪問するロケットの紹介です。</p><p>以下に、ロケットの例を2つ示します:<br>the <b><font color='${practice_info.font_colors[0]}'>${practice_info.rocket_names[0]}</font></b> and <b><font color='${practice_info.font_colors[1]}'>${practice_info.rocket_names[1]}</font></b> rocket ships.</p>`,
    `<p>各ロケットは「たいてい」は、到着する惑星がきまっています。</p><p>このロケットならば、あの宇宙人と出会える、と「たいてい」わかっているのです。</p><p></p>`,
    "<p>もし特定の惑星で、特定の宇宙人と出会いたいなら, そこへ連れていって<br>くれる可能性が最も高い色のロケットを選ぶ必要があります。</p>",
  ...
  on_start: function(trial) {
    ...
    if (jsPsych.data.get().filter({quiz: 2}).count() == 0) {
      trial.pages.unshift(
        "<p>いかがですか。どの宇宙人が、あなたにとって悪条件の取引（－１０円）が多いか、わかったのではないでしょうか？</p>"
      )
    }
...
const instructions_3a = {
  ...
  pages: [
    ...
    "<p>あなたの目的はいつ（時間の経過とともに）、どの宇宙人が、あなたにとって「悪条件の取引」をするか注意してみつけ、回避することです。</p>",
    "できる限り<b>悪条件の取引期間</b>を把握し、<b>マイナス１０円</b>を回避してください。<b>以下が３つの攻略手がかりです。</b></p><p>注意してお読みください。</p>",
    "<p><b>Hint #1:</b>宇宙人が、あなたにとって「悪条件の取引」をする確率は時間の経過とともに変化しますが、この変化はゆっくりです。そのため、どの宇宙人が、悪条件の取引をしているかを覚えておく必要があります。今あなたに悪条件の取引をする宇宙人は、しばらく続ける可能性が高いからです。</p>",
