...
const instructions_1a = {
  ...
  pages: [
    ...
    "<p>通訳の「ワープさん」は、</p><p>取引で損をすると「ぷんぷん顔」になります。</p><p>表情はこちらです。（画像は練習用です）</p>",
    ...
    "<p>あなたは今回、「やさしい宇宙群」と「きびしい宇宙群」のうち、「きびしい宇宙群」で取引するので、シビアな取引（－１０円）を避け、<b>謝礼金を減らさないことが目標です。</b></p>",
  ],
  ...
  on_start: function(trial) {
    ...
    if (jsPsych.data.get().filter({quiz: 1}).count() == 0) {
      trial.pages.unshift(
        "<p>これから「宇宙人との資源交換取引ゲーム」の<b>説明と体験を開始します。</b></p><p>下のNextボタンを使うか、キーボードの矢印キー<br>を使い、説明と体験を進めてください。</p>",
...
const instructions_1b_node = {
  timeline: [{
    type: jsPsychTwoStepInstructions,
    pages: [
      "<p>お見事です。実際に宇宙人と取引するには、最初に搭乗ロケットを選択し、到着さきの宇宙人を選択する２段階の選択が必要になりますが、ここでは先に後半部分の宇宙人との取引を体験しましょう。</p><p>次の画面で<b>左/右の矢印キーを使って</b>取引する宇宙人を選択します。</p><p>どの宇宙人が「あたりの顔」をみせる可能性が高いか試行してください。</p>",
...
const instructions_2a = {
  ...
  pages: [
    ...
    `<p>ここからは、前半部分の選択である、惑星を訪問するロケット選びも含めた、通しでの体験です。</p><p>以下に、ロケットの例を2つ示します:<br>the <b><font color='${practice_info.font_colors[0]}'>${practice_info.rocket_names[0]}</font></b> and <b><font color='${practice_info.font_colors[1]}'>${practice_info.rocket_names[1]}</font></b> rocket ships.</p>`,
