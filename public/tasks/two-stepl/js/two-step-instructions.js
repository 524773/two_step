...
const instructions_1a = {
  ...
  on_start: function(trial) {
    ...
    if (jsPsych.data.get().filter({quiz: 1}).count() == 0) {
      trial.pages.unshift(
        "<p>これから「宇宙人との資源交換取引ゲーム」の<b>インストラクションを開始します。</b></p><p>下のNextボタンを使うか、キーボードの矢印キー<br>を使い、インストラクションを進めてください。</p>",
...
const quiz_1 = {
  ...
  prompts: [
    ...
    "<i>True</i> or <i>False</i>:&nbsp;あなたの目標は、どの宇宙人がシビアな取引をする時期か、判断することです。",
...
const practice_1_help_node = {
  timeline: [{
    type: jsPsychTwoStepInstructions,
    pages: [
      "<p>お困りですか。</p><p>覚えておいてください。<b>たいていの場合</b>にシビアな取引をする宇宙人を<br><b>覚えるのです。</b></p>",
      "<p>もう一度やってみましょう。</p><p>次の画面で<b>左/右の矢印キーを使って</b>取引する宇宙人を選択します。どの宇宙人がシビアになるか判断するチャンスが 10 回あります。</p>",
    ],
...
const instructions_2a = {
  ...
  on_start: function(trial) {
    ...
    if (jsPsych.data.get().filter({quiz: 2}).count() == 0) {
      trial.pages.unshift(
        "<p>いかがですか。どの宇宙人がシビアになる可能性が高いか、わかったのではないでしょうか？</p>"
      )
    }
...
const instructions_3a = {
  ...
  pages: [
    ...
    "<p>あなたの目的はいつ（時間の経過とともに）、どの宇宙人が「シビアな取引」をするか注意してみつけることです。</p>",
    "今回あなたはシビアな取引の宇宙群なので、できる限り<b>プラスマイナス０</b>をめざし、シビアな取引<b>（マイナス１０円）</b>を回避してください。<b>以下が３つの攻略手がかりです。</b></p><p>注意してお読みください。</p>",
    "<p><b>Hint #1:</b>宇宙人が「シビアな取引」をする確率は時間の経過とともに変化しますが、この変化はゆっくりです。そのため、どの宇宙人があなたにシビアな取引をしているかを覚えておく必要があります。今あなたに「あたり」をみせている宇宙人は、しばらく「あたり」をみせ続ける可能性が高いからです。</p>",
  ...
  on_start: function(trial) {
    ...
    if (jsPsych.data.get().filter({quiz: 3}).count() == 0) {
      trial.pages.unshift(
        ...
        "<p>イージーな宇宙群に移動した方は謝礼金の基本は１，１００円です。</p>",
        "<p>シビアな宇宙群に移動した方は謝礼金の基本は１，９００円です。</p>",
        "<p>あなたは今回、シビアな宇宙群に運ばれますが、「どちらも同じ程度の金額」が期待されますので、がんばって下さい。</p>",
      )
    }
...
var INSTRUCTIONS_SKIP = {
  ...
  stimulus: '<p>これから、「宇宙人との資源交換取引ゲーム」の <b>説明とインストラクションが始まります。</b></p><p>インストラクションを始めるには, キーボードの「１」を押してください。</p>',
  choices: ["1"]
}
