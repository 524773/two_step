  ...
  **/
  class TwoStepTrialPlugin {
    ...
    trial(display_element, trial) {
      ...
      var state_1_ids = [0,1];
      const trialSide = getPositionBasedOnTrial(trial.trial_number);  // trial.trial_numberで現在のトライアル番号を取得

      if (trialSide === 'right') {
          state_1_ids = [1, 0]; // 右側にロケットを配置（逆順）
      } else {
          state_1_ids = [0, 1]; // 左側にロケットを配置（通常通り）
      }
      ...
      var state_2_ids = [0,1];
      if (trialSide === 'right') {
          state_2_ids = [1, 0]; // 右側にロケットを配置（逆順）
      } else {
          state_2_ids = [0, 1]; // 左側にロケットを配置（通常通り）
      }
